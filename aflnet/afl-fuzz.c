...
HANDLE_RESPONSES:
  ...
  // Do not spin on kill(child_pid, 0) here. A terminated child may remain as
  // a zombie until run_target() performs waitpid() immediately after
  // send_over_network() returns; kill(pid, 0) still succeeds for zombies.
...
EXP_ST void setup_shm(void) {
  ...
  reuse_shm = getenv("AFLNET_REUSE_SHM_ID");

  if (reuse_shm && !strcmp((char*)reuse_shm, "1") && getenv(SHM_ENV_VAR)) {

    shm_id = atoi(getenv(SHM_ENV_VAR));
    if (shm_id < 0) FATAL("Invalid existing SHM id in %s", SHM_ENV_VAR);
    ...
    switch (opt) {
      ...
      case 'P': /* protocol to be tested */
        ...
        if (!strcmp(optarg, "RTSP")) {
          ...
          extract_requests = &extract_requests_minecraft;
          extract_response_codes = &extract_response_codes_minecraft;
        } else {
