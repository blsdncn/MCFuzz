...
/* Read a VarInt from buf starting at offset.
   Returns the decoded value, and writes the number of bytes consumed into *bytes_read.
   Returns -1 on error (buffer too short or malformed VarInt). */
...
region_t* extract_requests_minecraft(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if (packet_end >= buf_size) {
      // Packet runs past end of buffer — include partial packet as final region
      packet_end = buf_size - 1;
    }
  ...
  // If no regions found but buffer is non-empty, treat entire buffer as one region
  if ((region_count == 0) && (buf_size > 0)) {
    regions = (region_t *)ck_realloc(regions, sizeof(region_t));
    regions[0].start_byte = 0;
    regions[0].end_byte = buf_size - 1;
...
unsigned int* extract_response_codes_minecraft(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  unsigned int *state_sequence = NULL;
  unsigned int state_count = 0;
  unsigned int byte_count = 0;
  ...
  while (byte_count < buf_size) {
    ...
    unsigned int packet_data_start = byte_count + len_bytes;
    if (packet_data_start >= buf_size) break;

    unsigned int id_bytes;
    int packet_id = read_varint(buf, buf_size, packet_data_start, &id_bytes);
    if (packet_id < 0 || id_bytes == 0) break;

    unsigned int message_code = get_mapped_message_code((u32)packet_id);

    state_count++;
    state_sequence = (unsigned int *)ck_realloc(state_sequence, state_count * sizeof(unsigned int));
    state_sequence[state_count - 1] = message_code;

    byte_count = packet_data_start + packet_len;
  }
