...
WORK_DIR="${WORK_DIR:-${ROOT_DIR}/build/jazzer}"
ARTIFACTS_DIR="${WORK_DIR}/artifacts"
LOG_DIR="${WORK_DIR}/logs"
CORPUS_DIR="${WORK_DIR}/corpus"
COVERAGE_DIR="${WORK_DIR}/coverage"
TARGET_CLASS="${TARGET_CLASS:-com.velocitypowered.proxy.fuzz.VelocityProtocolStateFuzzTarget}"
...
if [ ! -f "${ROOT_DIR}/proxy/build/classes/java/test/com/velocitypowered/proxy/fuzz/VelocityProtocolStateFuzzTarget.class" ]; then
...
if [ "${RESET_OUTPUTS}" = "1" ]; then
  ...
  rm -f "${ROOT_DIR}"/crash-* "${ROOT_DIR}"/slow-unit-*
fi
...
SEED_SCRIPT="${ROOT_DIR}/scripts/jazzer-seed-corpus.py"
if [ -f "${SEED_SCRIPT}" ]; then
  printf '[1/8] Seeding corpus with Velocity issue-inspired inputs...\n'
  python3 "${SEED_SCRIPT}"
else
