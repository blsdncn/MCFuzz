...
/**
 * Jazzer fuzz target that explores protocol state transitions and packet decoding paths.
 */
public final class VelocityProtocolStateFuzzTarget {
  ...
  private VelocityProtocolStateFuzzTarget() {
  }
  ...
  public static void fuzzerTestOneInput(byte[] input) {
    ...
    StateRegistry state = StateRegistry.HANDSHAKE;
    int steps = cursor.consumeInt(1, MAX_STEPS);

    for (int i = 0; i < steps; i++) {
      state = exploreState(state, cursor);
  ...
  static StateRegistry exploreState(StateRegistry currentState, InputCursor cursor) {
    int transition = cursor.consumeInt(0, 3);
    return switch (currentState) {
      case HANDSHAKE -> transition == 0 ? StateRegistry.STATUS : StateRegistry.LOGIN;
      case STATUS -> transition == 0 ? StateRegistry.HANDSHAKE : StateRegistry.STATUS;
      case LOGIN -> switch (transition) {
        case 0 -> StateRegistry.CONFIG;
        case 1 -> StateRegistry.PLAY;
        default -> StateRegistry.LOGIN;
      };
      case CONFIG -> switch (transition) {
        case 0 -> StateRegistry.PLAY;
        case 1 -> StateRegistry.LOGIN;
        default -> StateRegistry.CONFIG;
      };
      case PLAY -> switch (transition) {
        case 0 -> StateRegistry.CONFIG;
        case 1 -> StateRegistry.LOGIN;
