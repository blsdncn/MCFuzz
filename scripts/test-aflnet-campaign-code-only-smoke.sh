...
wait_for_port_free() {
  ...
  echo "port $port still in use before code-only campaign smoke" >&2
...
AFLNET_FEEDBACK_MODE=code-only \
...
grep -q '^aflnet_feedback_mode=code-only$' "$SUMMARY"
grep -q '^aflnet_state_aware_enabled=no$' "$SUMMARY"
grep -q '^aflnet_feedback_type=code-only$' "$SUMMARY"
grep -q '^edge_feedback_evidence=shm-attached$' "$SUMMARY"
grep -q '^agent_engine=ShmCoverageEngine$' "$SUMMARY"
...
echo "PASS: AFLNet campaign code-only smoke"
